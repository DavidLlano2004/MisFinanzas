...
## Flujo híbrido: VS Code para editar, Android Studio para emular

Si escribís el código en VS Code pero querés usar el emulador de Android Studio:

1. Abrí el proyecto en Android Studio (**File → Open** → seleccioná la carpeta del proyecto)
2. Android Studio detecta automáticamente el proyecto Gradle y sincroniza
3. Podés editar en VS Code y cuando quieras probar:
   - Iniciá el emulador desde Android Studio (**Device Manager → Play**)
   - Volvé a VS Code y ejecutá `./gradlew installDebug` desde la terminal
   - El APK se instala directamente en el emulador ya corriendo

Alternativamente, desde Android Studio podés hacer click en **Run** (▶) directamente y usa VS Code solo para editar.

> **Nota:** No modifiques archivos al mismo tiempo desde ambos IDEs. Guardá todos los cambios en VS Code antes de compilar desde Android Studio.
...
| Gradle no sincroniza en AS | **File → Sync Project with Gradle Files** |
| Emulador no arranca | Habilitar virtualización (Intel VT-x / AMD-V) en BIOS |
